...
func atualizar_barras_do_hud(delta: float):
	...
	if intervalo_atualizacao_barras <= 0.0 :
		if not player_dono:
			print("Não tem player status para atualizar barras visuais")
			return
		
		
		if player_dono.find_child("ComponentManager").Status.vida_atual != barra_vida_principal.value:
			barra_vida_principal.value = player_dono.find_child("ComponentManager").Status.vida_atual
			barra_vida_valor.text = "%.2f / %.2f" % [player_dono.find_child("ComponentManager").Status.vida_atual, player_dono.find_child("ComponentManager").Status.vida_max]
		
		if player_dono.find_child("ComponentManager").Status.vida_max != barra_vida_principal.max_value:
			barra_vida_principal.max_value = player_dono.find_child("ComponentManager").Status.vida_max
			barra_vida_valor.text = "%.2f / %.2f" % [player_dono.find_child("ComponentManager").Status.vida_atual, player_dono.find_child("ComponentManager").Status.vida_max]
